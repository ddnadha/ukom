            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="flex justify-between items-center">
                    ...
                    <a href="{{ route('item.create') }}"
                        class="rounded text-white bg-indigo-600 px-4 mx-4 py-2 text-xs font-medium text-white hover:bg-indigo-700">Tambah
                        Barang</a>
                </div>
                    ...
                    <table class="w-full divide-y divide-gray-200 bg-white text-sm">
                        <thead class="text-left">
                            <tr>
                                ...
                                <th class="whitespace-nowrap p-4 font-bold text-gray-900">Kategori</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach ($items as $item)
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap p-4 text-gray-900">@rupiah($item->price)</td>
                                    <td class="whitespace-nowrap p-4 text-gray-900">
                                        <img src="{{asset('storage/'.$item->img)}}" alt="" class="h-16">
                                    </td>
                                    <td class="whitespace-nowrap p-4 text-gray-900">{{ $item->category->name }}</td>
                                    <td class="whitespace-nowrap p-4">
                                        <form action="{{ route('item.destroy', $item) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a href="{{route('item.edit', $item)}}" class="inline-block rounded text-white bg-yellow-500 px-4 py-2 text-xs font-medium text-white hover:bg-yellow-600">Edit</a>
                                            <button type="submit"
